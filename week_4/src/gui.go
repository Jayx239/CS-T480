...
func gui() {
	...
	butparam := "-borderwidth 0 -font [list Clean 10] -padx 1 -pady 0"
	...
	if width > 480 {
		...
		fmt.Fprintf(gpipe, "place .cmode -x %d -y %d\n", width / 10 + 5, height - 82 + 5)
		fmt.Fprintf(gpipe, "place .copc -x %d -y %d\n", width / 10 + 5, height - 82 + 20)
		fmt.Fprintf(gpipe, "place .copp -x %d -y %d\n", width / 10 + 30, height - 82 + 20)
		fmt.Fprintf(gpipe, "place .copa -x %d -y %d\n", width / 10 + 55, height - 82 + 20)
		fmt.Fprintf(gpipe, "place .clbut -x %d -y %d\n", width / 10 + 47, height - 82 + 53)
		fmt.Fprintf(gpipe, "place .readbut -x %d -y %d\n", width / 10 + 47, height - 82 + 83)
		fmt.Fprintf(gpipe, "place .initbut -x %d -y %d\n", width / 10 + 47, height - 82 + 113)
		fmt.Fprintf(gpipe, "place .pulbut -x %d -y %d\n", width / 10 + 47, height - 82 + 143)
	} else {
		fmt.Fprintf(gpipe, "place .cmode -x %d -y %d\n", width / 10 + 5, height - 78)
		fmt.Fprintf(gpipe, "place .copc -x %d -y %d\n", width / 10 + 5, height - 65)
		fmt.Fprintf(gpipe, "place .copp -x %d -y %d\n", width / 10 + 30, height - 65)
		fmt.Fprintf(gpipe, "place .copa -x %d -y %d\n", width / 10 + 55, height - 65)
		fmt.Fprintf(gpipe, "place .clbut -x %d -y %d\n", width / 10 + 47, height - 52)
		fmt.Fprintf(gpipe, "place .readbut -x %d -y %d\n", width / 10 + 47, height - 31)
		fmt.Fprintf(gpipe, "place .initbut -x %d -y %d\n", width / 10 + 47, height - 10)
		fmt.Fprintf(gpipe, "place .pulbut -x %d -y %d\n", width / 10 + 47, height + 11)
	}
	fmt.Fprintln(gpipe, "button .resetbut -borderwidth 2 -font [list Clean 10] -text RESET " +
		"-command {puts stdout \"R\"}\n")
	fmt.Fprintln(gpipe, "button .loadbut -borderwidth 2 -font [list Clean 10] " +
	...
	fmt.Fprintf(gpipe, "button .s1but -borderwidth 2 -pady 2 -font [list Clean 10] " +
		"-text \"S1 View\" -command {puts stdout \"s1view\"}\n")
	fmt.Fprintf(gpipe, "button .s2but -borderwidth 2 -pady 2 -font [list Clean 10] " +
		"-text \"S2 View\" -command {puts stdout \"s2view\"}\n")
	fmt.Fprintf(gpipe, "button .s3but -borderwidth 2 -pady 2 -font [list Clean 10] " +
		"-text \"S3 View\" -command {puts stdout \"s3view\"}\n")
	fmt.Fprintf(gpipe, "button .s4but -borderwidth 2 -pady 2 -font [list Clean 10] " +
		"-text \"S4 View\" -command {puts stdout \"s4view\"}\n")
	fmt.Fprintf(gpipe, "button .s5but -borderwidth 2 -pady 2 -font [list Clean 10] " +
		"-text \"S5 View\" -command {puts stdout \"s5view\"}\n")
	fmt.Fprintf(gpipe, "button .perspbut -borderwidth 2 -pady 2 -font [list Clean 10] " +
		"-text \"Persp View\" -command {puts stdout \"perspview\"}\n")
	if width > 480 {
		fmt.Fprintf(gpipe, "place .s1but -x %d -y %d\n", width / 10 + 100, height)
		fmt.Fprintf(gpipe, "place .s2but -x %d -y %d\n", width / 10 + 100, height + 23)
		fmt.Fprintf(gpipe, "place .s3but -x %d -y %d\n", width / 10 + 100, height + 46)
		fmt.Fprintf(gpipe, "place .s4but -x %d -y %d\n", width / 10 + 100, height + 69)
		fmt.Fprintf(gpipe, "place .s5but -x %d -y %d\n", width / 10 + 100, height + 92)
		fmt.Fprintf(gpipe, "place .perspbut -x %d -y %d\n", width / 10 + 175, height + 92)
	} else {
...
func neonplcl(gpipe io.Writer, name string, cond bool, x, y int) {
	if cond {
		xp, yp, vis := ray(x, y, 1)
		if vis {
			fmt.Fprintf(gpipe, "place %s -x %d -y %d\n", name, xp, yp)
			return
		}
	}
