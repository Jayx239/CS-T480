...
func addlookup(f *ft, c int) {
	...
	if c & Fourp != 0 {
		for i := 0; i < 4; i++ {
			if x := f.cons[i]; x >= 4 && x <= 9 {
				a |= 1 << (9 - uint(i))
			}
			if x := f.cons[i+4]; x >= 4 && x <= 9 {
				b |= 1 << (9 - uint(i))
			}
		}
		for i := 0; i < 6; i++ {
			if x := f.tab[arg][i+1]; x >= 4 && x <= 9 {
				a |= 1 << (5 - uint(i))
			}
			if x := f.tab[arg][i+7]; x >= 4 && x <= 9 {
				b |= 1 << (5 - uint(i))
			}
		}
	}
	if c & Twopp != 0 {
		for i := 0; i < 4; i++ {
			if x := f.cons[i]; x == 8 || x == 9 {
				a |= 1 << (9 - uint(i))
			}
			if x := f.cons[i+4]; x == 8 || x == 9 {
				b |= 1 << (9 - uint(i))
			}
		}
		for i := 0; i < 6; i++ {
			if x := f.tab[arg][i+1]; x == 8 || x == 9 {
				a |= 1 << (5 - uint(i))
			}
			if x := f.tab[arg][i+7]; x == 8 || x == 9 {
				b |= 1 << (5 - uint(i))
			}
		}
	}
	if c & Twop != 0 {
		for i := 0; i < 4; i++ {
			if x := f.cons[i]; x == 2 || x == 3 || (x > 5 && x < 10) {
				a |= 1 << (9 - uint(i))
			}
			if x := f.cons[i+4]; x == 2 || x == 3 || (x > 5 && x < 10) {
				b |= 1 << (9 - uint(i))
			}
		}
		for i := 0; i < 6; i++ {
			if x := f.tab[arg][i+1]; x == 2 || x == 3 || (x > 5 && x < 10) {
				a |= 1 << (5 - uint(i))
			}
			if x := f.tab[arg][i+7]; x == 2 || x == 3 || (x > 5 && x < 10) {
				b |= 1 << (5 - uint(i))
			}
		}
	}
	if c & Onep != 0 {
		for i := 0; i < 4; i++ {
			if x := f.cons[i]; x < 10 && x % 2 == 1 {
				a |= 1 << (9 - uint(i))
			}
			if x := f.cons[i+4]; x < 10 && x % 2 == 1 {
				b |= 1 << (9 - uint(i))
			}
		}
		for i := 0; i < 6; i++ {
			if x := f.tab[arg][i+1]; x < 10 && x % 2 == 1 {
				a |= 1 << (5 - uint(i))
			}
			if x := f.tab[arg][i+7]; x < 10 && x % 2 == 1 {
				b |= 1 << (5 - uint(i))
			}
		}
	}
